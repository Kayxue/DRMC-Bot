        ...
        if (data.get("success").equals(true)) {
            ...
            suggestionchannel.retrieveMessageById(data.getLong("messageid")).queue(
                    message -> {
                        ...
                        message.editMessage(check.getAsMention()+" 建議已實施！").setEmbeds(embedBuilder.build()).queue(
                                editmessage -> {
                        ...
                        suggestionAuthor.openPrivateChannel().queue(
                                channel -> {
                                    channel.sendMessage("您提的建議已實施！原因為：" + reasonstring).setEmbeds(embedBuilder.build()).queue();
